...
public class FriendChatActivity extends AppCompatActivity {
    ...
    private void initView() {
        dip_margin = Utils.dip2px(this, 5);// 获取当前手机的像素密度（1个dp对应几个px）
        TextView tv_title = findViewById(R.id.tv_title);

        et_input = findViewById(R.id.et_input);//>>>啥时候聚焦?? 自动聚焦哦宝子
        sv_chat = findViewById(R.id.sv_chat);
        ll_show = findViewById(R.id.ll_show);

        findViewById(R.id.iv_back).setOnClickListener(v -> finish());
        findViewById(R.id.ib_img).setOnClickListener(v -> {//加号
    ...
    private void appendNowMinute() {
        ...
        if (!mMinute.substring(0, 4).equals(nowMinute.substring(0, 4))) {
            mMinute = nowMinute;
            ll_show.addView(ChatUtil.getHintView(this, nowMinute, dip_margin));//// 获得一个提示内容的文本视图模板 TextView tv
        }
    }
    ...
    // 接收对方传来的图片数据
    private void receiveImage(Object... args) { //to这个人的客户端收到了一个object的对象，属性有from(name) to(name) 内容(ImagePart)。
        JSONObject json = (JSONObject) args[0];
        ImageMessage message = new Gson().fromJson(json.toString(), ImageMessage.class); // 为啥不直接用args 用了message ??
    ...
    // 往聊天窗口添加图片消息
    private void appendChatImage(String name, String imagePath, boolean isSelf) { // appendChatImage(mSelfName, imagePath, true);
        appendNowMinute(); // 往聊天窗口添加当前时间 //ll_show.addView(
        // 把图片消息的线性布局添加到聊天窗口上
        ll_show.addView(ChatUtil.getChatImage(this, name, imagePath, isSelf));//ll_show.addView
        // 延迟100毫秒后启动聊天窗口的滚动任务
        new Handler(Looper.myLooper()).postDelayed(() -> {
            sv_chat.fullScroll(ScrollView.FOCUS_DOWN); // 滚动到底部
        }, 100);

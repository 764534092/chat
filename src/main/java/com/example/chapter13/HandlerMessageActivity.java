...
public class HandlerMessageActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private class PlayThread extends Thread {
        @Override
        public void run() { //无需显示调用，在启动线程实例时就会调用任务对象的run方法
            ...
            while (isPlaying) { // 正在播放新闻
                ...
                mHandler.sendMessage(message); // 分线程向处理器（主线程操作的哦）发送消息 ！！！！！！！！！！！！！
                // 将消息推送到当前时间之前的所有待处理消息之后的消息队列末尾。它将在 handleMessage 中接收，在附加到此处理程序的线程中。返回：如果消息已成功放入消息队列，则返回 true。失败时返回 false，通常是因为处理消息队列的循环程序正在退出。
            }
...
//            });
    ...
    // 创建一个处理器对象
    private Handler mHandler = new Handler(Looper.myLooper()) { //主线程定义处理器对象，主线程才能操作UI界面
        // 在收到消息时触发
        @Override

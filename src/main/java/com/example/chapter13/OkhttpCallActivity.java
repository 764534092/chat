...
public class OkhttpCallActivity extends AppCompatActivity { //怎么开启登陆接口啊？？？？？？？？ 手机和pc 连接同一个wifi
    private final static String TAG = "OkhttpCallActivity";
//    private final static String URL_STOCK = "https://fanyi.baidu.com";
...
//    private final static String URL_LOGIN = NetConst.HTTP_PREFIX;
    ...
    private void doGet() {
        ...
        call.enqueue(new Callback() { //分线程的？？
            @Override
    ...
    private void postForm() {
        ...
        OkHttpClient client = new OkHttpClient(); // 创建一个okhttp客户端对象
        // 创建一个POST方式的请求结构
        Request request = new Request.Builder().post(body).url(URL_LOGIN).build(); //请求中携带的数据为Request.post(body)
//        Request request = new Request.Builder().post(body).url("http://192.168.2.10:8080").build();
        Call call = client.newCall(request); // 根据请求结构创建调用对象
        // 加入HTTP请求队列。异步调用，并设置接口应答的回调方法
        call.enqueue(new Callback() {
            ...
            @Override
            public void onResponse(Call call, final Response response) throws IOException { // 请求成功  请求成功了但是返回的数据不对 ，服务器被动返回的是response数据
                String resp = response.body().string();
                // 回到主线程操纵界面
    ...
    private void postJson() {
        ...
        try {
            ...
            jsonObject.put("password", password); //postForm 是 body.add
        ...
        //异步则是可以提高效率了，现在cpu都是双核，四核，异步处理的话可以同时做多项工作，当然必须保证是可以并发处理！！！！！的
        //同步可以避免出现死锁，读脏数据的发生（有可能使一个人读取另一个人已经删除的内容）

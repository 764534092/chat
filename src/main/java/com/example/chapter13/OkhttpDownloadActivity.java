    ...
    private void downloadFile() {
        ...
        call.enqueue(new Callback() {
            ...
            public void onResponse(Call call, final Response response) { // 请求成功
                ...
                try (InputStream is = response.body().byteStream();  // 没有专门的解码工具，Bitmap bitmap = BitmapFactory.decodeStream(is); 只能老老实实地从输入流中读取字节数据到文件输出流中
                     FileOutputStream fos = new FileOutputStream(path)) {
                    byte[] buf = new byte[100 * 1024];
                    int sum=0, len=0;
                    while ((len = is.read(buf)) != -1) {
                        fos.write(buf, 0, len);// FileOutputStream
                        sum += len;

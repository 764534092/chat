...
import io.socket.client.IO; //IO.socket(uri) == Socket
...
public class SocketioTextActivity extends AppCompatActivity {
    ...
    private Socket mSocket; // 声明一个套接字对象 //为什么不用实例化？
//ws://localhost:8080/java/hello
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.btn_send).setOnClickListener(v -> {
            ...
            mSocket.emit("send_text", content); // 往Socket服务器发送文本消息 不用实例化吗，不是不用实例化，而是实例化在下面的！！！
        });
        initSocket(); // 初始化套接字
    }
    ...
    private void initSocket() {
        ...
        try {
            ...
            mSocket = IO.socket(uri); // 创建指定地址和端口的套接字实例  在这里初始化套接字，返回的是public 静态类，
        } catch (URISyntaxException e) {
            throw new RuntimeException(e);
        }
        mSocket.connect(); // 建立Socket连接
        // 等待接收传来的文本消息
        mSocket.on("receive_text", (args) -> { //为什么会多次执行？？？？？？？ 难道是异步？args表示从服务器传回来的消息

...
public class EntityListAdapter extends BaseAdapter {
    ...
    public EntityListAdapter(Context context, List<EntityInfo> user_list) { //  activity   类EntityInfo 的属性 name relation（string）
        mContext = context;
        mUserList = user_list;
    }
    ...
    // 定义一个视图持有者，以便重用列表项的视图资源，不用每次都findViewById

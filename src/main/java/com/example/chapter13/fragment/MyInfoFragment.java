...
public class MyInfoFragment extends Fragment { //自己为什么不主动 emit self_online ?? 自己不emit就没办法在后端弄那3个缓存啊 ，妈的你写到FriendListFragment里，不太好哦 宝贝
    ...
    private void initView() {
        ...
        mView.findViewById(R.id.iv_back).setOnClickListener(v -> getActivity().finish());//finish 返回到登录页面了
        iv_portrait = mView.findViewById(R.id.iv_portrait);
        tv_nick = mView.findViewById(R.id.tv_nick);
        mView.findViewById(R.id.ll_nick).setOnClickListener(v -> modifyNickName()); //OnClickListener l
    }
    ...
    private void showPortrait() {
        ...
        Drawable drawable = ChatUtil.getPortraitByName(mContext, nickName);//根据昵称自定义头像哦
